...
export function RightPanel({ tab, onTab, runtime, diff, trace }: { tab: Tab; onTab: (tab: Tab) => void; runtime: RuntimeView | undefined; diff?: DiffSnapshot; trace?: TracePage }) {
  ...
  return <aside className="right-panel">
    ...
    {tab === "trace" && <div className="panel-content"><h3>运行观测</h3><div className="stat">Agent 阶段 <strong>{runtime?.agentState ?? "REQUIREMENTS"}</strong></div><div className="stat">事件序号 <strong>{runtime?.lastSeq ?? 0}</strong></div>{traceItems.length ? <div className="trace-list">{traceItems.map((item, index) => <TraceEvent item={item} key={`${String(item.timestamp ?? "")}-${String(item.event ?? "event")}-${index}`} />)}</div> : <div className="muted">暂无事件</div>}</div>}
  </aside>;
}
